...
export default function AddBto() {
  ...
  const handleClose = () => {
    if (message==="Added project successfully."){
  ...
  const submitHandler = async () => {
    const obj = {
      ...
      unit_breakdown: [
        r2 ? r2UnitsRef.current.value : 0,
        r3 ? r3UnitsRef.current.value : 0,
        r4 ? r4UnitsRef.current.value : 0,
        r5 ? r5UnitsRef.current.value : 0,
        gen ? genUnitsRef.current.value : 0,
      ],
      price_range_2R: r2
        ? [priceLowR2Ref.current.value, priceHighR2Ref.current.value]
        : null,
      price_range_3R: r3
        ? [priceLowR3Ref.current.value, priceHighR3Ref.current.value]
        : null,
      price_range_4R: r4
        ? [priceLowR4Ref.current.value, priceHighR4Ref.current.value]
        : null,
      price_range_5R: r5
        ? [priceLowR5Ref.current.value, priceHighR5Ref.current.value]
        : null,
      price_range_3Gen: gen
        ? [priceLowGenRef.current.value, priceHighGenRef.current.value]
        : null,
    ...
    const result = await addProjectApi(obj, state);
    if (result.message) {
  ...
  return (
    <Container maxWidth="md">
      ...
      >
        Newly added projects are assumed to not have any admins and data
        scapers. Admin and data scrapers will be automatically added to project
